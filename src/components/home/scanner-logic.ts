// Scanner logic - lazy loaded when scanner section becomes visible
...
async function handleScannerSubmit(form: HTMLFormElement): Promise<void> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error ?? `Server-feil: ${String(response.status)}`);
    }
...
function setupTooltips(): void {
  ...
  document.querySelectorAll('.info-tooltip').forEach(btn => {
    ...
    currentScope.add(btn as HTMLElement, 'mouseenter', () => {
      ...
      tooltip.style.bottom = `${String(window.innerHeight - top)}px`;
...
function generateFeedback(data: AnalysisResult): string {
  ...
  const weakAreas: { label: string; score: number; priority: string }[] = [];
  const strongAreas: string[] = [];
  const okAreas: { label: string; score: number }[] = [];
